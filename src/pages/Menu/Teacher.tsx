import {
	...
	ChipField, Create,
	Datagrid,
	DateField, List, NumberInput, PasswordInput, ReferenceArrayInput, ReferenceField,
	ReferenceManyField, required,
	RichTextField, SelectArrayInput, SimpleFormIterator,
...
export const PanelTask = () => {
	return (
		<ReferenceField source="id" reference="tasks" link={false}>
			...
			<SimpleShowLayout>
				<ReferenceManyField source="id" reference="study_group_tasks" target="id"
				                    label="Assigned to Study Group(s)">
					<SingleFieldList>
						<ChipField source="study_group_cipher_id" style={{background: "#8758FF"}} icon={<GroupIcon/>}/>
					</SingleFieldList>
				</ReferenceManyField>
			</SimpleShowLayout>
			
			<SimpleShowLayout>
				<ReferenceManyField source="id" reference="student_tasks" target="id"
				                    label="Assigned to Student(s)">
					<SingleFieldList>
						<ReferenceField source="student_id" reference="users/role/student" emptyText="not set">
							<ChipField source={"full_name"} style={{background: "#8758FF"}} icon={<FaceIcon/>}/>
						</ReferenceField>
					</SingleFieldList>
				</ReferenceManyField>
			</SimpleShowLayout>
...
export const PanelTeacherTask = (props: any) => {
	return (
		<ListItem>
			<ReferenceManyField source="user_id"
		                    reference="tasks"
...
export const TeacherList = () => (
	<List>
		<Datagrid rowClick="show" bulkActionButtons={false} expand={<PanelTeacherTask/>} expandSingle >
...
export const TeacherCreate = () => (
	<Create>
      ...
      <TextInput source="role" defaultValue={"teacher"} label={"User Role"} disabled/>
      ...
      <TextInput source="phone" sx={user_sx} />
        ...
        <SimpleFormIterator inline>
          <ReferenceInput source="discipline_id" reference="disciplines" >
				      <AutocompleteInput source="title" optionText="title" optionValue="id" label='Disciplines'/>
				  </ReferenceInput>
	        <ReferenceInput source="role" reference="classifiers/teacher_role" >
						<AutocompleteInput source="id" optionText="id" label={"Teacher Role"}/>
					</ReferenceInput>
    ...
    <Create redirect="list">
      <SimpleForm>
        ...
        <TaskPriorityInput {...props} sx={task_sx} />
	      <ReferenceArrayInput source="study_group_cipher_id" reference="study_group_ciphers">
					<SelectArrayInput
						source="id"
						optionValue="id"
