import {
	...
	ChipField, Create,
	Datagrid, DateField,
	Edit, List, NumberInput, PasswordInput,
	ReferenceField,
	ReferenceInput,
	ReferenceManyField, required, RichTextField, SimpleForm,
...
export const PanelTask = () => (
	<ReferenceField source="id" reference="tasks" link={false} >
		<SimpleShowLayout>
			...
			<DateField source="completion_date" showTime={true} emptyText={"not set"}/>
		</SimpleShowLayout>
	</ReferenceField>
);
			...
			<ListItem >
				<Datagrid bulkActionButtons={false} expand={<PanelTask/>} expandSingle>
					...
					>
						...
						<ReferenceField source="teacher_user_id" reference="users/role/teacher">
							<ChipField source="email" style={{background: "#6358ff"}}/>
						</ReferenceField>
						<ChipField source="teacher_role" style={{background: "#9b58ff"}}/>
					</ReferenceField>
					<ChipField source="status"/>
					<DateField source="deadline_date" showTime={true} emptyText={"not set"}/>
				</Datagrid>
			</ListItem>
		</ReferenceManyField>
	)};
...
export const StudentList = () => {
	return (
		<List>
			<Datagrid rowClick="edit" bulkActionButtons={false} expand={PanelStudentTask} expandSingle>
				<ReferenceField source="id"
				                reference="users/role/student"
				                link={(record) => `/students/${record.id}/show`} label={"User"}
				>
					<TextField source="email"/>
				</ReferenceField>
				<ChipField source="role" label={"Role"}/>
				<ReferenceField source="study_group_cipher_id" reference="study_group_ciphers" label={"Study Group"}>
					<ChipField source="id"/>
				</ReferenceField>
			</Datagrid>
		</List>
...
export const StudentEdit = () => (
	<Edit>
		...
		<SimpleForm>
			<ReferenceInput source="role" reference="classifiers/student_role" label={"Student Role"}>
...
export const StudentCreate = () => (
	<Create>
		<SimpleForm>
			<TextInput source="email" sx={user_sx} validate={required()} defaultValue={null}/>
      <PasswordInput source="password" sx={user_sx} validate={required()}/>
      <TextInput source="username" sx={user_sx} validate={required()}/>
      <TextInput source="user_role" defaultValue={"student"} label={"User Role"} disabled/>
      ...
      <TextInput source="phone" sx={user_sx} />
			<ReferenceInput source="role" reference="classifiers/student_role" >
				<AutocompleteInput source="role" optionText="id" optionValue="id" label={"Student Role"}/>
			</ReferenceInput>
			<ReferenceInput source="study_group_cipher_id" reference="study_group_ciphers" >
